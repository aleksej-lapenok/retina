...
def focal(alpha=0.25, gamma=2.0, sigma_var=None):
    ...
    def _focal(y_true, y_pred):
        ...
        cross_entropy = keras.backend.binary_crossentropy(labels, classification) * keras.backend.exp(-sigma_var) + 2 / sigma_var
...
def smooth_l1(sigma=3.0, sigma_var=None):
    ...
    def _smooth_l1(y_true, y_pred):
        ...
        regression_loss = backend.where(
            ...
            factor * sigma_squared * keras.backend.pow(regression_diff, 2) + 0.5 * sigma_var,
            np.sqrt(2) * (regression_diff - 0.5 / sigma_squared) / keras.backend.sqrt(keras.backend.exp(sigma_var)) + 0.5 * sigma_var
        )
